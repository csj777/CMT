...
def add_avg_performance(dataset, result_dict):
    ...
    avg_class_names = [map_name_to_kitti[name] for name in avg_class_names]
    ...
    print(dataset_name)
    if dataset_name == 'KittiDataset' or dataset_name == 'NuScenesDataset' or dataset_name == 'KittiDatasetMT' or dataset_name == 'NuScenesDatasetMT':
...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None, args=None):
    ...
    for i, batch_dict in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            elif cfg.get('SELF_TRAIN', None) and cfg.SELF_TRAIN.get('MT', None):
                batch_dict = batch_dict[-1]
