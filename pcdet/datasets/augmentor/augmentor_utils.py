...
def global_rotation_wide(gt_boxes, points, rot_range):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    if enable:
        ...
        noise_rotation = np.random.uniform(-rot_range[1], -rot_range[0])
...
def scale_pre_object(gt_boxes, points, gt_boxes_mask, scale_perturb, num_try=50):
    ...
    for k in range(num_boxes):
        ...
        # obj_labels = beam_labels[point_masks > 0] if beam_labels is not None else None
