...
class DataAugmentor(object):
    ...
    def random_object_scaling(self, data_dict=None, config=None):
        ...
        no_ROS_gt_boxes_scale_noise = np.ones(no_ROS_gt_boxes.shape[0], dtype=np.float32)
        ...
        data_dict['points'] = points
        object_scale_noise = np.concatenate((no_ROS_gt_boxes_scale_noise, object_scale_noise))
        data_dict['object_scale_noise'] = object_scale_noise
    ...
    @staticmethod
    def label_point_cloud_beam(polar_image, points, beam=32):
    ...
    def random_world_rotation_wide(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_world_rotation, config=config)
        rot_range = config['WORLD_ROT_ANGLE']
        if not isinstance(rot_range, list):
            rot_range = [-rot_range, rot_range]
        gt_boxes, points, noise_rotation = augmentor_utils.global_rotation_wide(
            data_dict['gt_boxes'], data_dict['points'], rot_range=rot_range
        )
