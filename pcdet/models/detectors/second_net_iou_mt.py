...
class SECONDNetIoUMT(Detector3DTemplate):
    ...
    def forward(self, batch_dict, is_ema=True, cur_epoch=None, ema_model=None, return_batch_dict=False, no_nms=False):
        if is_ema:
            if isinstance(batch_dict, dict):
                batch_dict['dataset_cfg'] = self.dataset.dataset_cfg
                for cur_module in self.module_list:
                    try:
                        batch_dict = cur_module(batch_dict, no_nms=no_nms)
                    except:
                ...
                if return_batch_dict:
                    ...
                    if no_nms:
                        batch_dict['rois'] = batch_dict['rois_pre_nms']
                        batch_dict['roi_scores'] = batch_dict['roi_scores_pre_nms']
                        batch_dict['roi_labels'] = batch_dict['roi_labels_pre_nms']

                    pred_dicts, recall_dicts = self.post_processing_multicriterion(batch_dict, no_nms=no_nms)
